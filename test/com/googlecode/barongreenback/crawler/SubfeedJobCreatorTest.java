...
//        Sequence<HttpJob> jobs = SubfeedJobCreator.createSubfeedJobs(one(record().set(LINK, URI)), SOME_DESTINATION, sequence(record()));
...
//        HttpJob job = SubfeedJobCreator.job(LINK, record().set(LINK, URI), SOME_DESTINATION, sequence(record()));
...
//        Sequence<Record> records = SubfeedJobCreator.mergePreviousUniqueIdentifiers(one(record().set(PERSON_NAME, "Dan")), SubfeedDatasource.dataSource(null, null, one(Pair.<Keyword<?>, Object>pair(LINK, URI))));
