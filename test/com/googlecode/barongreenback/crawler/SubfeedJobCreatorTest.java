...
public class SubfeedJobCreatorTest {
    ...
    public void ifRecordContainsSubfeedReturnsJob() throws Exception {
        ...
        Sequence<StagedJob> jobs = new SubfeedJobCreator(SOME_DESTINATION, new HashSet<HttpDatasource>(), crawlerId, record()).process(one(record().set(LINK, URI))).second();
    ...
    public void shouldPassDownKeyAndValuesToSubfeedJobs() throws Exception {
        ...
        UUID crawlerId = UUID.randomUUID();
        Sequence<StagedJob> jobs = new SubfeedJobCreator(SOME_DESTINATION, new HashSet<HttpDatasource>(), crawlerId, previousUnique).process(one(record().set(LINK, URI))).second();
    ...
    public void shouldMergeUniqueKeysIntoEachRecord() throws Exception {
        ...
        Pair<Sequence<Record>, Sequence<StagedJob>> records = new SubfeedJobCreator(
                SOME_DESTINATION, new HashSet<HttpDatasource>(), crawlerId, previousRecord).process(one(record().set(PERSON_NAME, "Dan")));
