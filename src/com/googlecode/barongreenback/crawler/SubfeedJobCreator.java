...
public class SubfeedJobCreator {
    ...
    public static Sequence<StagedJob<Response>> createSubfeedJobs(final Container container, Sequence<Record> records, Definition destination, Sequence<Pair<Keyword<?>, Object>> data) {
        Sequence<StagedJob<Response>> realise = records.flatMap(subfeedsKeywords(container, destination, data)).unique(uri()).realise();
        for (StagedJob<Response> job : realise) {
    ...
    private static Callable1<Record, Sequence<StagedJob<Response>>> subfeedsKeywords(final Container container, final Definition destination, final Sequence<Pair<Keyword<?>, Object>> data) {
    ...
    private static Callable1<StagedJob<Response>, Uri> uri() {
        return new Callable1<StagedJob<Response>, Uri>() {
            @Override
            public Uri call(StagedJob<Response> job) throws Exception {
                return job.dataSource().uri();
            }
        };
    }

    private static Callable1<Keyword<?>, StagedJob<Response>> toJob(final Container container, final Record record, final Definition destination, final Sequence<Pair<Keyword<?>, Object>> data) {
